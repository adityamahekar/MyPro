// Theme toggle functionality for UIUX page
function toggleTheme() {
    const body = document.body;
    const isDarkMode = body.classList.contains('dark-mode');
    const themeToggles = document.querySelectorAll('#themeToggle, #uiuxThemeToggle');
...
// Add animation to UIUX cards when they come into view
...
function handleImageErrors() {
    const images = document.querySelectorAll('.uiux-image img');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!document.querySelector('#uiux-animations')) {
        const style = document.createElement('style');
        style.id = 'uiux-animations';
        style.textContent = `
            @keyframes fadeInUp {
                from {
                    opacity: 0;
...
window.addEventListener('resize', function() {
    ...
    if (window.innerWidth <= 768) {
        if (uiuxGrid) {
            uiuxGrid.style.gridTemplateColumns = '1fr';
        }
    } else {
        if (uiuxGrid) {
            uiuxGrid.style.gridTemplateColumns = 'repeat(auto-fit, minmax(400px, 1fr))';
        }
    }
});